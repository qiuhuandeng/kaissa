...
run('monthly-profit',async({page,url,download,shot,passed})=>{
  ...
  await selectQueryView(page, 'departments');assert.match(await h.locator('[data-fr-main]').innerText(),/-600.00/);
  await selectQueryView(page, 'budgets');assert.match(await h.locator('[data-fr-main]').innerText(),/-100.00/);passed('公司部门费用守恒、零负预算和全依据导出');
  await selectQueryView(page, 'group');assert.match(await h.locator('[data-fr-main]').innerText(),/未确认/);
  ...
  await selectQueryView(page, 'companies');await h.locator('[name=version]').selectOption('corrected');await h.locator('button[type=submit]').click();assert.match(await h.locator('[data-fr-main]').innerText(),/18,500.00/);
