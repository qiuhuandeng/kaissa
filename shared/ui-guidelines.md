...
- 右抽屉必须先按任务分型，不使用一个通用模板硬套所有业务场景：
  ...
  - 复杂创建/长流程类使用 `.drawer-modal.drawer-flow` 或完整页面，用于新建线路、新建团期、新建订单、创建独立成团、新建定制需求、开排航次等跨多个业务对象的流程。内容按真实业务流程分段，可使用步骤条或分段导航；未被明确要求重构时，新建线路、新建团期、新建订单保持独立，不借全局抽屉调整顺手改动。
