...
- 右抽屉必须先按任务分型，不使用一个通用模板硬套所有业务场景：
  ...
  - 预览/速览类使用 `.drawer-modal.drawer-preview`，用于订单、团期、单据、日志、版本、记录等快速判断场景。顶部必须有对象摘要和主状态，主体以描述项、指标、轻量表格、时间线为主，底部只保留“进入详情 / 进入处理页面 / 关闭”等少量入口，不承载复杂编辑。
  - 处理/审核类使用 `.drawer-modal.drawer-action`，用于确认资源、调名额、申请付款、审批、异常处理、发送通知等推进状态的操作。顶部先交代对象和当前卡点，主体先展示判断依据，再展示本次处理项，底部主按钮必须使用业务动作词，如“确认占用 / 提交申请 / 审核通过 / 执行付款”。
- 右抽屉推荐结构为：`.modal-header` 内使用 `.drawer-title-stack` / `.drawer-title-row` 承载标题、上下文和主状态；`.modal-body` 内优先使用 `.drawer-object-summary` 表达当前业务对象，再按 `.drawer-section` 分区；`.modal-footer` 内可使用 `.drawer-footer-note` 和 `.drawer-footer-actions` 表达影响说明与按钮组。
