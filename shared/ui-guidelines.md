...
- 总原则：标题区只识别动作；摘要卡识别业务对象；字段区承载业务判断；底部只完成操作。说明性文案删除，业务上下文保留并放进内容区摘要卡。
- 新建、编辑、配置、批量维护、选择类型等承载表单或复杂内容的操作，使用 `.modal-overlay.drawer-overlay > .modal.drawer-modal`。
- 右抽屉必须先按任务分型，不使用一个通用模板硬套所有业务场景：
  - 创建/编辑类使用 `.drawer-modal.drawer-edit`，用于新增、编辑、维护、配置等以填写字段为主的操作。标题为“动作 + 业务对象”，主体以表单分组为主，必填项和本次影响字段靠前，底部主按钮为“保存 / 确认保存 / 提交”。
  - 预览/速览类使用 `.drawer-modal.drawer-preview`，用于订单、团期、单据、日志、版本、记录等快速判断场景。主体以对象摘要、描述项、指标、轻量表格、时间线为主，底部只保留“进入详情 / 进入处理页面 / 关闭”等少量入口，不承载复杂编辑。
  - 处理/审核类使用 `.drawer-modal.drawer-action`，用于确认资源、调名额、申请付款、审批、异常处理、发送通知等推进状态的操作。主体先展示业务对象和判断依据，再展示本次处理项，底部主按钮必须使用业务动作词，如“确认占用 / 提交申请 / 审核通过 / 执行付款”。
- 右抽屉标题区只放一个标题和关闭按钮，不放类型小标题、副标题、说明文案、业务摘要、状态标签或上下游信息。团期编号、出发日期、订单号、供应商、金额、主状态等业务内容必须放在 `.modal-body` 的 `.drawer-object-summary` 或后续业务分区内。
- 右抽屉正文保持业务字段和业务分区，不放“用于……”“这里只……”“当前抽屉不……”这类解释性说明。分区标题只写业务名词，例如“基础信息”“重点订单”“差异依据”“本次处理”。
- `.drawer-object-summary` 内允许在对象标题下放一行高价值业务上下文，例如“供应商 / 费用项目 / 关联团期/线路/班期”。这类上下文必须是当前处理判断所需的业务信息，不是解释说明；金额、数量、状态等重点短值仍应拆成摘要字段单独展示。
- 右抽屉底部只放操作按钮，不放流程说明、影响说明或提示句；必要的风险和结果应通过业务字段、状态标签或处理后的 toast 表达。
- 右抽屉推荐结构为：`.modal-header` 承载 `.modal-title` 和关闭按钮；`.modal-body` 内优先使用 `.drawer-object-summary` 表达当前业务对象，再按 `.drawer-section` 分区；`.modal-footer` 内使用 `.drawer-footer-actions` 承载按钮组。
