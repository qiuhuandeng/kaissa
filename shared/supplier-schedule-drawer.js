(function () {
  ...
  function priceTitle(type) {
    if (type === "邮轮") return "舱型结算价";
    if (type === "专列") return "铺位结算价";
    if (type === "自由行") return "套餐结算价";
    if (type === "研学") return "营期结算价";
    return "人群结算价";
  }
  ...
  function priceRows(product) {
    if (!product || product.type === "参团游") {
      return [["成人结算价", numericPrice(product && product.price)], ["儿童结算价", "4980"], ["单房差", "2000"], ["服务费/补差", "0"]];
    }
    if (product.type === "邮轮") return [["内舱结算价", numericPrice(product.price)], ["阳台结算价", "10900"], ["套房结算价", "16800"], ["港务费", "0"]];
    if (product.type === "专列") return [["硬卧结算价", "16800"], ["软卧结算价", numericPrice(product.price)], ["单铺差", "0"], ["服务费/补差", "0"]];
    if (product.type === "自由行") return [["成人结算价", numericPrice(product.price)], ["儿童结算价", "3280"], ["单房差", "0"], ["服务费/补差", "0"]];
    return [["学生结算价", numericPrice(product.price)], ["陪同结算价", "6800"], ["单房差", "0"], ["服务费/补差", "0"]];
  }

  function drawerHtml(products) {
    var cards = products.map(function (product, index) {
      return [
        '<button class="batch-product-card' + (index === 0 ? " active" : "") + '" type="button" data-product-index="' + index + '">',
        '  <div class="batch-product-head"><strong>' + escapeHtml(product.product) + '</strong><span class="tag ' + escapeHtml(product.tagClass) + '">' + escapeHtml(product.type) + '</span></div>',
        '  <div class="batch-product-meta"><span>' + escapeHtml(product.destination || "线路") + '</span><span>' + escapeHtml(product.days + "天") + '</span><span>' + escapeHtml(product.meta || objectNameByType(product.type)) + '</span></div>',
        '</button>'
      ].join("");
    }).join("");
    return [
      ...
      '          <div class="batch-flow-head"><span class="batch-flow-index">1</span><div><h2>选择产品线路</h2></div></div>',
      ...
      '          <div class="batch-route-picker-panel">',
      '            <div class="batch-product-select-wrap"><div class="batch-product-select"><span aria-hidden="true">⌕</span><input id="batchProductSearch" type="search" autocomplete="off" placeholder="搜索线路名称" aria-label="搜索线路名称"><button id="clearBatchProductSearch" class="batch-product-clear" type="button" aria-label="清除线路搜索" hidden>×</button><div id="batchProductInlinePanel" class="batch-picker-panel" hidden><div id="batchProductList" class="batch-product-list" aria-label="产品卡片列表">' + cards + '</div><div id="batchProductEmpty" class="batch-product-empty" hidden>未找到匹配线路</div></div></div></div>',
      '            <div id="batchPlanSelectWrap" class="batch-plan-select-wrap" hidden><label class="form-label" for="batchRoutePlanCards">选择产品线路 <span class="req">*</span></label><select id="batchRoutePlan" class="batch-route-hidden" aria-hidden="true" tabindex="-1"></select><div id="batchRoutePlanCards" class="batch-plan-card-list" aria-label="产品线路"></div><div id="batchRoutePlanEmpty" class="batch-product-empty" hidden>未找到匹配线路</div></div>',
      '          </div>',
      '        </section>',
      '        <section class="batch-flow-card">',
      ...
      '            <div id="batchExcelPanel" class="batch-date-panel batch-excel-panel" hidden><button id="batchExcelUpload" class="batch-excel-upload" type="button"><span class="batch-excel-upload-icon" aria-hidden="true">↥</span><strong>点击自动模拟上传 Excel 排班文件</strong><em>支持读取：出发日期、默认余量、结算价备注字段</em></button><div class="batch-excel-actions"><button id="batchExcelTemplate" class="btn btn-secondary" type="button">下载 Excel 模板</button><button id="batchExcelSample" class="btn btn-secondary" type="button">载入示例排期数据（3条）</button></div><div id="batchExcelStatus" class="form-hint">尚未导入排班数据。</div></div>',
      ...
      '          </div></div>',
      '        </section>',
      '        <section class="batch-flow-card"><div class="batch-flow-head"><span class="batch-flow-index">3</span><div><h2>名额与结算价</h2></div></div><div class="form-grid schedule-quota-price-grid"><div class="form-group"><label class="form-label" for="batchCapacityLimit">计划收客</label><input id="batchCapacityLimit" class="form-control" type="number" min="1" value="20"></div><div class="form-group"><label class="form-label" for="batchMinGroupSize">成人数</label><input id="batchMinGroupSize" class="form-control" type="number" min="1" value="12"></div><div class="form-group"><label class="form-label" for="batchConfirmMode">确认方式</label><select id="batchConfirmMode" class="form-control"><option>下单后二次确认</option><option>凯撒预占后确认</option><option>固定余量可直接售卖</option></select></div><div class="form-group"><label class="form-label" for="batchValidUntil">价格有效期</label><input id="batchValidUntil" class="form-control" type="date" value="2026-07-01"></div></div><section id="batchTypeSpecificPanel" class="batch-type-panel" aria-label="结算价结构"><div class="batch-section-subhead"><h3 id="batchTypeSpecificTitle">结算价结构</h3><span id="batchInventoryModeTag" class="tag tag-blue">按人售卖</span></div><div id="batchTypeSpecificGrid" class="batch-type-grid"></div></section></section>',
      ...
      '      </section>',
      '      <section id="batchPreviewPage" class="batch-preview-page" aria-label="新建团期预览确认" hidden><div class="batch-preview-title"><div><h2>预览确认</h2></div><span id="batchCreateCountText">将提交 0 个团期</span></div><section class="batch-preview-summary" aria-label="新建团期摘要"><div class="batch-preview-summary-main"><span>产品/线路</span><strong id="batchPreviewRouteText">-</strong></div><div><span>默认计划收客</span><strong id="batchPreviewCapacityText">-</strong></div><div><span>结算价</span><strong id="batchPreviewPriceText">-</strong></div><div><span>确认方式</span><strong id="batchPreviewPolicyText">-</strong></div><div><span>凯撒对接</span><strong id="batchPreviewLeaderText">-</strong></div></section><section class="table-wrap batch-preview-table drawer-table-fit drawer-table-cols-5" aria-label="即将提交的团期预览"><table><thead><tr><th>出发日期</th><th>计划收客</th><th>结算价</th><th>确认方式</th><th>操作</th></tr></thead><tbody id="batchPreviewRows"></tbody></table></section></section>',
      '    </div>',
  ...
  function initDrawer(options) {
    ...
    var manualDates = new Set(["2026-07-10", "2026-07-24", "2026-08-07"]);
    ...
    function renderPlans(product, selectedRoute) {
      ...
      planCards.innerHTML = list.map(function (name, index) {
        ...
        return '<button class="batch-plan-card' + (active ? " active" : "") + '" type="button" data-plan="' + escapeHtml(name) + '"><span class="batch-plan-main"><strong>' + escapeHtml(name) + '</strong><em>' + escapeHtml(product.destination || "") + '</em></span><span class="batch-plan-days">' + escapeHtml(product.days + "天") + '</span></button>';
      }).join("");
      selectedRouteName.textContent = currentPlan();
    }

    function renderPriceGrid(product) {
      document.getElementById("batchTypeSpecificTitle").textContent = priceTitle(product.type);
      document.getElementById("batchInventoryModeTag").textContent = inventoryMode(product.type);
      document.getElementById("batchTypeSpecificGrid").innerHTML = priceRows(product).map(function (row) {
        return '<label class="form-group"><span class="form-label">' + escapeHtml(row[0]) + '</span><input class="form-control supplier-batch-price-input" type="number" min="0" value="' + escapeHtml(row[1]) + '"></label>';
      }).join("");
    }

    function applyProduct(product, editMode, selectedRoute, customTitle) {
      cards().forEach(function (card) { card.classList.toggle("active", productByCard(card) === product); });
      productSearch.value = product.product;
      ...
      renderPlans(product, selectedRoute || product.route);
    ...
    function filterProducts(keyword) {
      ...
      cards().forEach(function (card) {
        var product = productByCard(card);
        var text = [product.product, product.route, product.type, product.destination].join(" ").toLowerCase();
        var matched = !value || text.indexOf(value) >= 0;
        card.hidden = !matched;
        if (matched) count += 1;
      });
      productEmpty.hidden = count > 0;
    }
    ...
    function renderCalendars(startValue, months, sets) {
      var start = new Date((startValue || "2026-07-01") + "T00:00:00");
      var html = [];
    ...
    function calcGeneratedDates() {
      ...
      if (mode === "excel") return ["2026-07-09", "2026-07-23", "2026-08-06"];
    ...
    function renderDates() {
      ...
      document.getElementById("batchDateResultBlock").hidden = mode === "manual";
    ...
    function firstPrice() {
      var input = document.querySelector(".supplier-batch-price-input");
      return "¥" + Number(input ? input.value || 0 : 0).toLocaleString("zh-CN");
    }
    ...
    function renderPreview() {
      ...
      document.getElementById("batchPreviewPriceText").textContent = firstPrice();
      document.getElementById("batchPreviewPolicyText").textContent = document.getElementById("batchConfirmMode").value;
      document.getElementById("batchPreviewLeaderText").textContent = document.getElementById("batchPlannerSelect").value;
      document.getElementById("batchPreviewRows").innerHTML = dates.map(function (date) {
        return '<tr data-preview-date="' + escapeHtml(date) + '"><td>' + escapeHtml(date) + '</td><td>' + escapeHtml(document.getElementById("batchCapacityLimit").value + product.unit) + '</td><td>' + escapeHtml(firstPrice()) + '</td><td>' + escapeHtml(document.getElementById("batchConfirmMode").value) + '</td><td><button class="table-action-primary" type="button" data-remove-preview-date="' + escapeHtml(date) + '">移除</button></td></tr>';
      }).join("") || '<tr><td colspan="5">暂无可提交日期</td></tr>';
    }
    ...
    document.addEventListener("click", function (event) {
      ...
      if (card) {
        applyProduct(productByCard(card), false);
        return;
      }
    ...
    productSearch.addEventListener("focus", function () { if (!productSearch.readOnly) productPanel.hidden = false; });
