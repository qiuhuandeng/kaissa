(function () {
  ...
  function demoAllowedModules() {
    ...
    const allowed = phases.operator && phases.operator[phase];
  ...
  function rootTitleFromActive(active) {
    if (!active) return "";
    return active.parent ? active.parent.title : active.item.title;
  }
  ...
  function resolveNavigationTarget(href) {
    return new URL(href || window.location.href, currentRouteUrl || window.location.href);
  }
  ...
  function createDemoUnavailablePanel(url) {
    ...
    home.href = menu[0] && menu[0].href ? menu[0].href : "dashboard.html";
  ...
  const metaTagTexts = new Set([
    ...
    "自营", "自营线路", "外采", "外采线路", "外采团期", "外采包团", "外采系列团", "邮轮", "邮轮线路", "专列", "专列线路", "研学", "研学线路", "委托单",
    "出境游", "国内游", "国内游", "港澳台", "小包团", "定制团", "半自由行", "出境游", "出境游", "普通报价", "团队报价",
    ...
    "线路经理", "线路总监", "销售顾问", "高级顾问", "顾问", "计调", "客服", "财务", "管理层", "门店店长", "呼叫中心坐席", "审批管理员", "超管",
    "专业版", "旗舰版", "基础版", "组织账号", "权限安全", "AI模型", "系统配置", "审计只读", "线路文案生成", "行程生成", "话术建议", "财务异常分析", "客户洞察", "其他"
  ]);

  const metaTagPatterns = [
    /^(自营|外采|邮轮|专列|研学|自由行|单项委托|出境游|出境游|国内游|MICE)(线路|团期)?$/,
  ...
  function classifyTableColumn(title, index, headerCell) {
    ...
    if (/线路\/|线路名称|线路名称|线路\/|资源名称|供应商名称|客户\/渠道|客户\/联系人|项目\/客户|项目名称|公司名称|门店名称|商户名称|租户名称|组织名称|部门名称|角色名称|员工账号|船只名称|船名|船公司名称|酒店名称|餐厅名称|车型名称|套餐名称|模板名称|规则名称|策略名称|任务名称/.test(text)) return "main";
  ...
  function loadDemoConfig(callback) {
    ...
    const script = document.createElement("script");
    const scriptBase = new URL(bootScript.src || "../shared/nav-operator.js", window.location.href);
