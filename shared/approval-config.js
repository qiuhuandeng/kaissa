(function () {
  ...
  function mountEditor(host){
    const params=new URLSearchParams(location.search);let id=params.get('id')||M.uid(),record=state.templates.find(x=>x.id===id),readonly=params.get('mode')==='view',step=2,dirty=false,flowZoom=100;
    ...
    function render(){
      ...
      <div class="ac-track" id="acEditorPanel" role="tabpanel" aria-labelledby="acTab${step}"><div id="acEditorErrors">${storageError?errorsHTML([storageError]):''}</div>${step===2?`<section class="ac-flow-workspace"><div class="ac-flow-zoom" role="group" aria-label="流程缩放"><button type="button" data-ac="zoom-out" aria-label="缩小流程" title="缩小" ${flowZoom<=50?'disabled':''}>−</button><output data-flow-zoom aria-live="polite">${flowZoom}%</output><button type="button" data-ac="zoom-in" aria-label="放大流程" title="放大" ${flowZoom>=150?'disabled':''}>+</button></div><div class="ac-flow"><div class="ac-flow-canvas" style="--ac-flow-scale:${flowZoom/100}">${flowForm()}</div></div></section>`:`<section class="ac-card ac-editor-form">${[scopeForm,contentForm,flowForm,rulesForm][step]()}</section>`}</div></div>`;
    ...
    host.addEventListener('click',ev=>{const b=ev.target.closest('[data-ac]');if(!b)return;const a=b.dataset.ac;
      if(a==='zoom-in'||a==='zoom-out'){flowZoom=Math.max(50,Math.min(150,flowZoom+(a==='zoom-in'?10:-10)));host.querySelector('.ac-flow-canvas').style.setProperty('--ac-flow-scale',flowZoom/100);host.querySelector('[data-flow-zoom]').textContent=flowZoom+'%';host.querySelector('[data-ac=zoom-out]').disabled=flowZoom<=50;host.querySelector('[data-ac=zoom-in]').disabled=flowZoom>=150;return;}
