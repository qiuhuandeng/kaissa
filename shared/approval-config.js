(function () {
  ...
  function mountEditor(host){
    const params=new URLSearchParams(location.search);let id=params.get('id')||M.uid(),record=state.templates.find(x=>x.id===id),readonly=params.get('mode')==='view',step=0,dirty=false;
    ...
    function render(){
      ...
      <div class="ac-editor-workspace"><nav class="ac-steps list-surface-tabs list-function-tabs" role="tablist" aria-label="审批模板配置">${['基础信息','审批内容','审批流程','审批规则'].map((s,i)=>`<button type="button" id="acTab${i}" class="tab-item ${i===step?'active':''}" role="tab" aria-selected="${i===step}" aria-controls="acEditorPanel" data-ac="step" data-step="${i}"><span class="list-tab-label">${s}</span></button>`).join('')}</nav>
      <div class="ac-track" id="acEditorPanel" role="tabpanel" aria-labelledby="acTab${step}"><div id="acEditorErrors">${storageError?errorsHTML([storageError]):''}</div>${step===2?`<div class="ac-flow-toolbar"><h2>审批流程</h2>${button('流程试跑','trial')}</div><section class="ac-flow">${flowForm()}</section>`:`<section class="ac-card ac-editor-form">${[scopeForm,contentForm,flowForm,rulesForm][step]()}</section>`}</div></div>`;
    ...
    function scopeForm(){return `<section class="ac-editor-section"><div class="ac-form">${field('模板名称 <span class="ac-required">*</span>',input('name',t.name,'data-field="name" maxlength="60" placeholder="例如：付款申请流程"'))}${field('业务场景 <span class="ac-required">*</span>',select('scene',M.scenes.map(s=>s.name),t.scene,'data-field="scene"'))}</div></section><section class="ac-editor-section"><h2>适用范围</h2><div class="ac-form"><div class="ac-field ac-wide"><span>${e(M.scene(t.scene).companyLabel)} <span class="ac-required">*</span></span><div class="ac-options">${M.companies.map(c=>check('company',e(c.name),t.companies.includes(c.id),`data-company="${c.id}"`)).join('')}</div></div><div class="ac-field ac-wide"><span>适用部门</span><div class="ac-options">${[['all','全部部门'],['selected','指定部门']].map(([value,label])=>`<label class="ac-check"><input type="radio" name="departmentMode" value="${value}" data-field="departmentMode" ${t.departmentMode===value?'checked':''}>${label}</label>`).join('')}</div><div class="ac-department-selection" ${t.departmentMode==='selected'?'':'hidden'}><div class="ac-options" data-department-options>${M.departments.filter(d=>t.companies.includes(d.company)).map(d=>check('department',e(M.companyName(d.company)+' / '+d.name),t.departments.includes(d.id),`data-department="${d.id}"`)).join('')||'<span class="ac-muted">请先选择适用公司</span>'}</div>${check('children','包含下级部门',t.includeChildren,'data-field="includeChildren"')}</div></div></div></section>`;}
    ...
    function flowForm(){return `<div class="ac-node ac-applicant-card"><div class="ac-node-head">申请人</div><button class="ac-node-body" data-ac="step" data-step="0"><span>${e(scopeText(t))}<span class="ac-muted">${t.departments.length?e(t.departments.map(id=>M.departments.find(d=>d.id===id)?.name||id).join("、")):"全部部门"}</span></span><span class="ac-card-chevron" aria-hidden="true">›</span></button></div>${flowList(t.nodes,'root')}<div class="ac-flow-end">审批结束</div>`;}
    ...
    function flowList(nodes,list){return addConnector(list,0)+nodes.map((n,i)=>{
      ...
      const controls=readonly?'':n.type==='branch'||nodes.length>1?link('更多','node-more',attrs):link('删除','node-delete',attrs);
      if(n.type==='branch')return `<div class="ac-branch-group" data-node-id="${n.id}" style="--ac-branch-count:${n.branches.length}" aria-label="${e(n.title)}"><div class="ac-branch-group-head ac-forkbar"><span class="sr-only">${e(n.title)}</span>${readonly?'<span class="ac-fork-label">条件分流</span>':button('添加条件','branch-add',`data-id="${n.id}"`)}<div class="ac-fork-more">${controls}</div></div><div class="ac-branches">${n.branches.map((b,j)=>`<div class="ac-branch" data-branch-id="${b.id}"><section class="ac-condition-card${b.fallback?' ac-default-card':''}"><div class="ac-condition-card-head">${b.fallback?`<span>默认条件</span><span>优先级 ${j+1}</span>`:`${readonly?`<span>${e(b.title)}</span>`:`<button type="button" class="ac-condition-name" data-ac="rename-condition" data-group="${n.id}" data-id="${b.id}" aria-label="重命名${e(b.title)}">${e(b.title)} <span aria-hidden="true">✎</span></button>`}<div class="ac-condition-card-actions"><span class="ac-condition-priority">优先级 ${j+1}</span>${readonly?'':`<button type="button" data-ac="copy-condition" data-group="${n.id}" data-id="${b.id}" aria-label="复制${e(b.title)}" title="复制条件及内部审批节点">复制</button>${n.branches.length>2?`<button type="button" data-ac="delete-condition" data-group="${n.id}" data-id="${b.id}" aria-label="删除${e(b.title)}" title="删除此条件分支">×</button>`:''}`}</div>`}</div>${b.fallback?'<div class="ac-branch-title ac-fallback">未满足其他条件时，进入此流程</div>':`<button class="ac-branch-title" data-ac="edit-condition" data-group="${n.id}" data-id="${b.id}"><span>${e(b.conditions.map(M.conditionText).join(b.match==='any'?' 或 ':' 且 '))||'请设置条件'}</span><span class="ac-card-chevron" aria-hidden="true">›</span></button>`}</section>${flowList(b.nodes,b.id)}<div class="ac-branch-footer"></div></div>`).join('')}</div></div>`+addConnector(list,i+1);
      return `<div class="ac-node ac-node-${n.type}" data-node-id="${n.id}"><div class="ac-node-head"><span>${n.type==='cc'?'抄送人':'审批人'}</span>${controls}</div><button class="ac-node-body" data-ac="edit-node" data-id="${n.id}"><span><strong>${e(n.title)}</strong><span class="ac-muted">${e(n.source==='duty'?n.duty:n.source==='member'?(names(n.members)||'请选择人员'):n.source==='business'?M.scene(t.scene).businessLabel:M.sources[n.source])+(n.hierarchy?' · '+(n.hierarchy.mode==='continuous'?'逐级至':'指定')+(n.hierarchy.origin==='top'?'最高层起':'')+'第'+n.hierarchy.level+'级'+(n.source==='manager'?'':' · 同层'+(n.hierarchy.within==='any'?'或签':'会签')):(n.type==='approval'?' · '+M.modes[n.mode]:''))}</span></span><span class="ac-card-chevron" aria-hidden="true">›</span></button></div>`+addConnector(list,i+1);
    }).join('');}
    function rulesForm(){
      const r=t.rules;
    ...
    function save(){if(!t.name.trim()){showErrors(host.querySelector('#acEditorErrors'),['请填写模板名称']);return false;}if(t.departmentMode==='selected'&&!t.departments.length){showErrors(host.querySelector('#acEditorErrors'),['请至少选择一个适用部门']);return false;}const next=M.clone(state);M.saveDraft(next,id,t);if(!persist(next))return false;record=state.templates.find(x=>x.id===id);dirty=false;try{history.replaceState(null,'',editorUrl(id));}catch(_){}render();toast('草稿已保存');return true;}
    host.addEventListener('input',ev=>{const k=ev.target.dataset.field;if(k==='name'){t.name=ev.target.value;dirty=true;}if(k==='hours'){t.rules.hours=ev.target.value;dirty=true;}});
    host.addEventListener('change',ev=>{
      ...
      if(el.dataset.company){if(el.checked)t.companies.push(el.dataset.company);else t.companies=t.companies.filter(x=>x!==el.dataset.company);t.departments=t.departments.filter(id=>M.departments.some(d=>d.id===id&&t.companies.includes(d.company)));render();return;}
      if(el.dataset.department){if(el.checked)t.departments.push(el.dataset.department);else t.departments=t.departments.filter(x=>x!==el.dataset.department);return;}
      if(k==='scene'){const previous=t.scene,value=el.value;el.value=previous;confirmAction('切换业务场景？','切换后将重新带入审批内容和流程示例。',()=>{const fresh=M.sample(value);t={...fresh,name:t.name,companies:t.companies,departments:t.departments,includeChildren:t.includeChildren,departmentMode:t.departmentMode};render();});return;}
      if(k==='departmentMode'){t.departmentMode=el.value;if(el.value==='all')t.departments=[];render();return;}
      if(k==='includeChildren')t.includeChildren=el.checked;
    ...
    host.addEventListener('click',ev=>{const b=ev.target.closest('[data-ac]');if(!b)return;const a=b.dataset.ac;
      ...
      function removeNode(){const list=M.getList(t,b.dataset.list),i=list?.findIndex(x=>x.id===b.dataset.id);if(i<0||!list)return;confirmAction('删除节点？',list[i].type==='branch'?'此分支组及内部审批节点将一并删除。':'删除后请重新试跑流程。',()=>{list.splice(i,1);dirty=true;render();});}
      if(a==='node-delete'&&!readonly)removeNode();
      if(a==='node-more'&&!readonly){const n=M.getNode(t,b.dataset.id),list=M.getList(t,b.dataset.list),items=[];
        if(n.type==='branch')items.push(['rename','分支名称']);
        if(list.length>1)items.push(['move','移动位置']);
        if(n.type==='branch'&&n.branches.filter(x=>!x.fallback).length>1)items.push(['order','调整条件顺序']);
        items.push(['delete',n.type==='branch'?'删除分支组':'删除节点']);
        menu(b,items,action=>{if(action==='rename')renameBranchGroup(n);if(action==='move')moveNodeDrawer(n,b.dataset.list);if(action==='order')branchOrderDrawer(n);if(action==='delete')removeNode();});
      }
    ...
    function deleteCondition(group,branch,after){if(branch.fallback||group.branches.length<3)return;confirmAction('删除条件分支？','该条件及其内部审批节点将一并删除，默认条件和其他分支保持。',()=>{group.branches=group.branches.filter(x=>x.id!==branch.id);dirty=true;if(after)after();render();});}
    ...
    function nodeDrawer(original,readOnly,onSave){
      ...
      function personControls(){
        ...
        if(n.source==='member'){
          ...
          const empty=!t.companies.length?'请先在“基础信息”选择适用公司':n.type==='cc'?'适用公司暂无有效任职员工，请先在“员工账号”维护任职':'适用公司暂无可审批成员，请先核对员工任职及审批权限';
