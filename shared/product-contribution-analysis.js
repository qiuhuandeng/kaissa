(function(root,factory){
  if(typeof module==='object'&&module.exports)module.exports=factory(require('./monthly-profit-model.js'));
    ...
    const labels={company:'核算公司',group:channel?'成交渠道':'产品分析项',currency:'原币',income:'确认收入',cost:'结转成本',gross:'业务毛利',fees:'已归属直接费用',contribution:'直接经营贡献',status:'资料情况',profitShare:'其中已计分润成本',commission:'渠道佣金',platform:'平台费',discount:'尚未扣除优惠',direct:'其他直接费用',id:'完成记录号',order:'订单号',product:'产品组合',destination:'目的地',type:'业务类型',supply:'供应方式',channel:'成交渠道',customer:'合同付款客户',external:'对外/内部'};
    root.CaesarReadonlyReport.mount(host,{
      ...
      money:['income','cost','gross','fees','contribution','commission','platform','discount','direct','profitShare'],
      columns:()=>api.columns,extras:['commission','platform','discount','direct','profitShare'],
      ...
      definition:'按核算公司、会计期间与原币分别核对。直接经营贡献=确认收入-结转成本-已归属且尚未扣除的直接费用；其中已计分润成本是直接费用的组成，不在直接费用之外再扣一次；仅引用已确认费用记录，不用分润预估或支付金额补成本。部门公共费用尚无逐产品或渠道分配资料，不推算完整利润。汇总与组成记录使用同一批完成记录和确认资料；全公司会计及费用依据归核算核对和月度损益。内部供货和对外销售不跨公司重复相加。'
    });
  }
