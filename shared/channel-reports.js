(function () {
  ...
  function mount(report) {
    ...
    function render() {
      ...
      root.querySelector('[data-result]').innerHTML = '<div class="report-section-head"><h2>' + esc(viewName(applied) + (applied.granularity === 'records' ? '明细' : '汇总')) + '</h2></div><details class="report-columns"><summary>显示字段</summary><div class="report-column-options">' + extra.filter(k => !(applied.granularity === 'records' ? ['company'] : baseColumns()).includes(k)).map(k => '<label><input type="checkbox" data-column="' + k + '"' + (optional.has(k) ? ' checked' : '') + '>' + m.labels[k] + '</label>').join('') + '</div></details>' +
