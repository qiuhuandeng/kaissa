(function () {
  ...
  function createModel(report) {
    ...
    function groupKeys(f) {
      ...
      return { channel: ['channel'], region: ['budgetRegion', 'channel'], company: ['company'], department: ['company', 'salesDivision', 'salesDepartment'] }[f.level];
    }
  ...
  function mount(report) {
    ...
    const date = key => '<label class="report-field"><span>' + (key === 'start' ? '开始日期' : '结束日期') + '</span><input type="date" name="' + key + '" value="' + draft[key] + '" required' + (key === 'start' && draft.period !== 'custom' ? ' disabled' : '') + '></label>';
    function choices(key) {
      ...
      return [['', '全部'], ...values.map(v => [v, key === 'company' || key === 'productCompany' ? v + '公司（演示）' : key === 'productOrg' ? v + '产品经营组' : key === 'salesId' ? data.find(r => r.salesId === v).salesperson + ' · ' + v : v]), ['__missing', '待补充']];
    }
    const field = key => select(key, m.labels[key], choices(key), draft[key]);
    function filters() {
      root.querySelector('[data-filters]').innerHTML = '<div class="report-filter-row">' + select('basis', '业绩口径', [['orders', '订单成交'], ['actual', '实际回团']], draft.basis) + select('period', '统计期间', [['custom', '自选期间'], ['seven', '近7日'], ['month', '月累计'], ['year', '年累计']], draft.period) + date('start') + date('end') + field('company') +
    ...
    function baseColumns() {
      ...
      if (applied.view === 'calls' && applied.callLevel === 'person') keys.push('salesperson');
    ...
    function columns() { return [...new Set([...baseColumns(), ...optional])]; }
    function header(key) { return (key === 'amount' ? applied.basis === 'orders' ? '净成交额' : '回团金额' : m.labels[key]) + (['amount', 'allAmount'].includes(key) ? '（' + (applied.unit === 'wan' ? '万元' : '元') + '）' : ''); }
    ...
    function sorted() { return [...result.summaries].sort((a, b) => { const av = a[sort], bv = b[sort]; if (av == null || bv == null) return av == null ? bv == null ? 0 : 1 : -1; return (typeof av === 'number' ? av - bv : String(av).localeCompare(String(bv), 'zh-CN')) * direction; }); }
    ...
    function render() {
      ...
      root.querySelector('[data-meta]').innerHTML = '<span class="report-demo">演示工作版 V1 · 数据截止 ' + report.CUTOFF + ' 23:59</span><span>' + m.views[applied.view] + ' · ' + (applied.basis === 'orders' ? '订单确认日期' : '实际完成日期') + ' ' + applied.start + ' 至 ' + applied.end + '</span><span>' + (applied.channel || '全部主渠道') + ' · 人民币' + (applied.unit === 'wan' ? '万元' : '元') + '</span>';
      ...
      root.querySelector('[data-result]').innerHTML = '<div class="report-section-head"><h2>' + (applied.basis === 'actual' ? '回团渠道分析 · ' : '订单渠道分析 · ') + m.views[applied.view] + '</h2><span class="report-muted" data-total>' + totalText() + '</span></div><details class="report-columns"><summary>显示字段</summary><div class="report-column-options">' + extra.filter(k => !baseColumns().includes(k)).map(k => '<label><input type="checkbox" data-column="' + k + '"' + (optional.has(k) ? ' checked' : '') + '>' + m.labels[k] + '</label>').join('') + '</div></details>' +
        table(data.slice((page - 1) * size, page * size), cols) + '<div class="report-total"><span>' + data.length + ' 行汇总 · 订单数按全查询去重</span><div class="report-pagination">' + select('size', '每页条数', [5, 10, 20, 50].map(n => [String(n), n + '条/页']), String(size)) + button(icon('chevron-left'), 'data-page="-1" aria-label="上一页" title="上一页"' + (page === 1 ? ' disabled' : '')) + '<span>' + page + ' / ' + pages + '</span>' + button(icon('chevron-right'), 'data-page="1" aria-label="下一页" title="下一页"' + (page === pages ? ' disabled' : '')) + '</div></div>' +
        '<details class="cr-source"><summary>来源明细（' + result.rows.length + '条）</summary>' + table(result.details, detailColumns, false) + '</details>';
    }
    ...
    root.addEventListener('click', e => {
      ...
      if (el.hasAttribute('data-reset')) { applied = { ...m.defaults, view: draft.view }; applied = m.clean(applied); draft = { ...applied }; optional = new Set(); page = 1; shell(); }
      ...
      if (el.hasAttribute('data-export')) {
        ...
        const a = document.createElement('a'); a.href = url; a.download = '渠道经营-' + m.views[applied.view] + '-演示.csv'; a.click(); setTimeout(() => URL.revokeObjectURL(url), 1000);
      }
    });
    shell();
