(function () {
  ...
  const config = {
    'performance-reports': ['经营总览', [...views(primary, [['primary','经营规模']]), ...views('[data-overview-finance]', [['profit','损益摘要'],['funds','资金概况'],['plan','计划概况'],['resources','风险概况']])]],
    'monthly-profit-reports': ['月度损益', views('[data-monthly-profit]', [['companies','公司损益'],['departments','部门损益'],['expenses','费用构成'],['budgets','预算差异'],['group','集团调整']])],
    'product-reports': ['产品分析', [...views(primary, [['organizations','经营业绩'],['structure','产品结构'],['channels','渠道构成'],['crossYear','跨年收客']]), ...views('[data-contribution="product"]', [['product','经营贡献']]), ...views('[data-resource-cost]', [['resources','产品风险']])]],
    'channel-reports': ['渠道分析', [...views(channel, [['channels','渠道概况'],['stores','门店业绩'],['calls','呼叫中心'],['structure','产品构成']]), ...views('[data-contribution="channel"]', [['channel','经营贡献']]), { selector: '[data-channel-margin]', key: 'margin', label: '毛利校验', owner: channel }]],
    ...
    'cashflow-reports': ['收付明细', views('#finance-cashflow', [['receipt','收款'],['refund','退款'],['transfer','转款'],['payment','付款']])],
    'balance-reports': ['往来账龄', views('#finance-balances', [['ar','应收余额'],['ap','应付余额'],['aging','账龄分析'],['clearing','内部清算']])],
  ...
  function persist() {
    if (active && !switching) {
      ...
      try { sessionStorage.setItem(storageKey, JSON.stringify({ active: active.key, states })); } catch (_) { /* In-memory navigation still works. */ }
    }
  }
